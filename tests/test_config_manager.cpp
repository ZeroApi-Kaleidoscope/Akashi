...
private Q_SLOTS:
  void init() { m_config = new ServerConfig(); }
...
void test_ConfigManager::verifyServerConfig()
{
  ...
  QCOMPARE(m_config->verifyServerConfig(), true);
  ...
  QCOMPARE(m_config->verifyServerConfig(), false);
...
{
  // We check that the list is unsorted and exactly as defined in the text file.
  QStringList l_backgrounds = m_config->backgroundList();
...
{
  MusicList l_musiclist = m_config->musicAlbumList();
  qDebug() << l_musiclist;
...
{
  QStringList l_ordered_musiclist = m_config->orderedMusicAlbumList();
...
void test_ConfigManager::regression_pr_314()
{
  ...
  Q_UNUSED(m_config->musicAlbumList());
  QCOMPARE(l_list.size(), m_config->orderedMusicAlbumList().size());
  QCOMPARE(l_list, m_config->orderedMusicAlbumList());
}

void test_ConfigManager::CommandInfo()
{
  // Prepare command help cache.
  m_config->loadCommandHelp();
  ServerConfig::CommandHelp l_help;

  // Load the sample command information.
  l_help = m_config->commandHelp("foo");
  QCOMPARE(l_help.text, "A sample explanation.");
  QCOMPARE(l_help.usage, "/foo <bar> [baz|qux]");

  l_help = m_config->commandHelp("login");
...
{
  QStringList l_ipranges = m_config->ipRangeBanList();
