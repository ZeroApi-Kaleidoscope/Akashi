...
void AOClient::loginAttempt(QString message)
{
  ...
  case DataTypes::AuthType::SIMPLE:
    ...
    Q_EMIT logLogin((character() + " " + characterName()), name(), "Moderator", m_ipid, server->getAreaById(areaId())->name(), m_authenticated);
    break;
  case DataTypes::AuthType::ADVANCED:
    ...
    Q_EMIT logLogin((character() + " " + characterName()), name(), username, m_ipid, server->getAreaById(areaId())->name(), m_authenticated);
    break;
  }
  sendServerMessage("Exiting login prompt.");
  m_is_logging_in = false;
